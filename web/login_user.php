...
<!-- **********************************************************************************************************************************************************
 ...
 <header class="header black-bg">
            ...
            <!--logo start-->
            <a href="index.php" class="logo"><b>Via Jaya Ac</b></a>
            <!--logo end-->
...
<div id="login-page">
  <div class="container">
    <form class="form-login" method="post" action="login_user.php?auth=''">
      <h2 class="form-login-heading">sign in now</h2>
      <div class="login-wrap">
        <input type="text" class="form-control" name="username" placeholder="USERNAME" autofocus>
        <br>
        <input type="password" name="password" class="form-control" placeholder="Password">
        <label class="checkbox"> <span class="pull-right"> <a data-toggle="modal" href="register.php"> Daftar Akun !</a> </span> </label>
        <button class="btn btn-theme btn-block" type="submit"><i class="fa fa-lock"></i> SIGN IN</button> </form>
        <?php
	  if(isset($_GET['auth']))
	  {
		  ...
		  if(mysql_num_rows($query)>0)
		  {
			  ...
			  {
				  mysql_query("UPDATE tbl_user SET last_login = '".date("Y-m-d H:i:s")."' WHERE id = ".$data["id"]."");
				  
				  $_SESSION['login_username'] = $data['username'];
				  $_SESSION['login_alamat'] = $data['alamat'];
				  $_SESSION['login_no_hp'] = $data['no_hp'];
	  ...
	  if(isset($_GET['getPassword']))
	  {
		  ...
		  if(mysql_num_rows($query)>0)
		  {
			 ...
			 mysql_query("UPDATE tbl_user SET password_md5='".md5($newPassword)."',password_cetak='".$newPassword."' WHERE id = '".$data['id']."'");
			 ?>
             <script>
